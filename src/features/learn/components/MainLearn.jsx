...
import { unlockTopic } from '../../user-profile/state/UserProgressReducer.jsx';
...
export default function MainLearn({ children }) {
   ...
   const isLevelUnlocked = (jlptLevel, topicName, levelType) => {
      ...
      return userProgress.completedLevels[jlptLevel]?.[topicName]?.includes(
         previousLevel
      );
   };
   ...
   const hasEnoughXP = (jlptLevel, topicName) => {
      ...
      const requiredXP =
         ListQuestionFakeDataLession[jlptLevel]?.[topicName]?.metadata
            ?.requiredXp || 0;
   ...
   const tryUnlockTopic = (jlptLevel, topicName) => {
      if (
         !isTopicUnlocked(jlptLevel, topicName) &&
         hasEnoughXP(jlptLevel, topicName)
      ) {
         ...
         if (lessonType === 'level1' || lessonType === 'theory') {
            if (!tryUnlockTopic(selectedJlptLevel, selectedTopic)) {
               ...
               const requiredXP =
                  ListQuestionFakeDataLession[selectedJlptLevel]?.[
                     selectedTopic
                  ]?.metadata?.requiredXp || 0;
               alert(
                  `Bạn cần ${requiredXP} XP để mở khóa chủ đề này. Hiện tại bạn có ${userProgress.experience} XP.`
               );
               return;
            }
         } else {
   ...
   return (
      <section className="section-1">
         ...
         <div className="line-study d-flex justify-content-start align-items-center">
            <ul className="step-course">
               {topicLessons.map((lessonType, index) => {
                  ...
                  return (
                     <li
                        key={lessonType}
                        className={`course position-relative ${clickedLessonIndex === index ? 'click' : ''} ${!isUnlocked ? 'locked' : ''}`}
                     ...
                     >
                        ...
                        <div className="tooltiptext">
                           ...
                           {isUnlocked ? (
                              <button
                                 onClick={() =>
                                    onHandleStartLession(lessonType)
                                 }
                              >
