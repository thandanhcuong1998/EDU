...
export const useLessionHook = () => {
   ...
   useEffect(() => {
      if (isCorrect === false) {
         setIncorrectAttempts(previous => previous + 1);
      }
   }, [isCorrect]);
   ...
   const getNextLevel = () => {
      ...
      const currentLevelNumber = parseInt(currentLevelMatch[1]);
      const nextLevelNumber = currentLevelNumber + 1;
      const nextLevelType = `level${nextLevelNumber}`;
   ...
   const createCombinedQuestions = nextLevelInfo => {
      ...
      const currentLevelNumber = parseInt(currentLevelMatch[1]);
      const previousLevelType = `level${currentLevelNumber - 1}`;

      // Check if previous level exists
      if (
         ListQuestionFakeDataLession[level] &&
         ListQuestionFakeDataLession[level][topic] &&
         ListQuestionFakeDataLession[level][topic][previousLevelType]
      ) {
         const previousLevelQuestions =
            ListQuestionFakeDataLession[level][topic][previousLevelType];

         // Select a subset of review questions (e.g., 30% of previous level)
         const reviewCount = Math.max(
            1,
            Math.floor(previousLevelQuestions.length * 0.3)
         );
         ...
         while (
            ...
            indices.size < previousLevelQuestions.length
         ) {
            const randomIndex = Math.floor(
               Math.random() * previousLevelQuestions.length
            );
            if (!indices.has(randomIndex)) {
               indices.add(randomIndex);
               // Create a copy of the question to avoid reference issues
               const questionCopy = JSON.parse(
                  JSON.stringify(previousLevelQuestions[randomIndex])
               );
