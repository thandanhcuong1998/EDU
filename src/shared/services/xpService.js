...
class XPService {
   ...
   addXP(userId, action, metadata = {}) {
      ...
      if (isWeekend && action !== 'loginBonus') {
         finalXP = Math.round(
            finalXP * this.defaultSettings.xpRewards.weekendBonus
         );
      }
      ...
      if (action === 'questionCorrect' && userData.streak > 0) {
         const streakBonus = Math.min(
            userData.streak * this.defaultSettings.xpRewards.streakBonus,
            50
         );
         finalXP += streakBonus;
      }
      ...
      userData.rankOrder = this.getRank(userData.level);
    ...
    */
   calculateLevel(xp) {
      const { baseXP, xpMultiplier, maxLevel } =
         this.defaultSettings.levelConfig;
   ...
   getLevelUpInfo(currentLevel, currentXP) {
      ...
      const xpProgress =
         ((currentXP - totalRequiredXP) /
            (nextLevelRequiredXP - totalRequiredXP)) *
         100;
   ...
   updateDailyStreak(userId) {
      ...
      if (daysDiff === 1) {
         ...
         userData.maxDailyStreak = Math.max(
            userData.maxDailyStreak,
            userData.dailyStreak
         );
      } else if (daysDiff > 1) {
         // Break in streak
   ...
   updateStats(userData, action, metadata) {
      switch (action) {
         case 'lessonCompleted':
            ...
            userData.stats.lessonsCompletedToday =
               (userData.stats.lessonsCompletedToday || 0) + 1;
            userData.stats.lessonsCompletedThisWeek =
               (userData.stats.lessonsCompletedThisWeek || 0) + 1;
            if (metadata.perfectScore) {
               userData.stats.perfectScoresToday =
                  (userData.stats.perfectScoresToday || 0) + 1;
            }
            break;
         ...
         case 'srsReview':
            ...
            userData.stats.srsReviewsToday =
               (userData.stats.srsReviewsToday || 0) + 1;
            userData.stats.srsReviewsThisWeek =
               (userData.stats.srsReviewsThisWeek || 0) + 1;
            break;
         case 'dailyGoalCompleted':
            userData.stats.dailyGoalsCompleted++;
            break;
         case 'xpEarned':
            userData.stats.xpEarnedToday =
               (userData.stats.xpEarnedToday || 0) + (metadata.amount || 0);
            userData.stats.xpEarnedThisWeek =
               (userData.stats.xpEarnedThisWeek || 0) + (metadata.amount || 0);
            break;
         case 'studyTime':
            userData.stats.studyTimeToday =
               (userData.stats.studyTimeToday || 0) + (metadata.minutes || 0);
            break;
      }
   }
   ...
   checkAchievements(userData) {
      ...
      // First lesson
      if (
         userData.stats.lessonsCompleted === 1 &&
         !existingAchievementIds.includes('first_lesson')
      ) {
      ...
      if (
         userData.streak >= 10 &&
         !existingAchievementIds.includes('perfect_score')
      ) {
         newAchievements.push({
            id: 'perfect_score',
            name: 'Hoàn hảo',
      ...
      // Week streak
      if (
         userData.dailyStreak >= 7 &&
         !existingAchievementIds.includes('week_streak')
      ) {
         newAchievements.push({
            id: 'week_streak',
            name: 'Kiên trì',
      ...
      // Month streak
      if (
         userData.dailyStreak >= 30 &&
         !existingAchievementIds.includes('month_streak')
      ) {
         newAchievements.push({
            id: 'month_streak',
            name: 'Bậc thầy',
      ...
      for (const milestone of levelMilestones) {
         if (
            userData.level >= milestone &&
            !existingAchievementIds.includes(`level_${milestone}`)
         ) {
            newAchievements.push({
               id: `level_${milestone}`,
